...
This script does NOT modify run_yaw_rate_control_suite.py nor config.yaml in-place.
It can either:
...
def _run_and_get_history(config_path: Path, out_dir: Path, tag_prefix: str) -> Path:
    ...
    script_path = Path(__file__).with_name("run_yaw_rate_control_suite.py").resolve()
...
def main() -> int:
    ...
    if b_first is None:
        ...
        # run_yaw_rate_control_suite.py updates B using *actual* steering axis torque; we store motor torque,
